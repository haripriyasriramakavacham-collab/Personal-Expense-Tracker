...
class ExpenseTrackerGUI(ExpenseReporter):
    ...
    def setup_ui(self):
        ...
        categories = ["Food", "Transport", "Entertainment", "Shopping", "Health", "Fixed Expenses","Other"]
        self.category_entry = ttk.Combobox(input_frame, values=categories, width=15, state="readonly")
        self.category_entry.set(categories[0]) 
        ...
        ttk.Label(input_frame, text="Description:").grid(row=1, column=2, sticky="e", padx=5, pady=5)
        ...
        ttk.Button(btn_frame, text="Add Expense", command=self.add_gui).pack(side="left", padx=4)
        ttk.Button(btn_frame, text="View Record", command=self.view_record_gui).pack(side="left", padx=4)
        ttk.Button(btn_frame, text="Update Selected", command=self.edit_gui).pack(side="left", padx=4)
        ttk.Button(btn_frame, text="Delete Selected", command=self.delete_gui).pack(side="left", padx=4)
        ttk.Button(btn_frame, text="Clear Form", command=self.clear_entries).pack(side="left", padx=4)
        ...
        ttk.Label(bottom_frame, text="Monthly Salary:").pack(side="left", padx=(20, 5))
        self.salary_entry = ttk.Entry(bottom_frame, width=12)
        self.salary_entry.pack(side="left", padx=5)
        saved_salary=self.load_salary_data()
        self.salary_entry.insert(0, f"{saved_salary:.2f}")
        self.salary_entry.bind("<KeyRelease>",self.on_salary_change)
    ...
    def load_salary_data(self):
        """Loads saved salary and month string (YYYY-MM)."""
        ...
        if os.path.exists("salary.txt"):
            try:
                with open("salary.txt", "r") as f:
                    ...
                    if len(content) == 2:
                        saved_month, saved_salary = content[0], float(content[1])
    ...
    def save_salary_data(self, salary_val):
        """Saves current month (YYYY-MM) and salary together."""
        current_month = datetime.now().strftime("%Y-%m")
        with open("salary.txt", "w") as f:
            f.write(f"{current_month},{salary_val}")

    def on_salary_change(self, event=None):
        """Saves salary with current month stamp and refreshes table."""
        try:
            salary = float(self.salary_entry.get().strip())
            self.save_salary_data(salary)
        except ValueError:
            pass
        self.refresh_table()
